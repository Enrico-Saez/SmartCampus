...
<div class="grid grid-cols-4 justify-items-center gap-y-10">
	{#each Array(10) as _, i}
		...
		>
			<div class="relative">
				<svg
					class="relative z-20 ml-2 mt-3 text-neutral-900 dark:text-slate-200"
				...
				<div
					class="lampada absolute left-[21%] top-[26%] h-9 w-7 rounded-3xl bg-[#ffe770]"
				></div>
			</div>
			<div
				class="absolute bottom-0 right-0 z-20 hidden h-[4.15rem] w-full bg-black/60 dark:block"
			></div>
			<div class="absolute bottom-2 right-3 z-30 text-right text-cyan-950 dark:text-yellow-100">
				<h1 class="text-xl font-semibold">
					<span class="mr-2 text-base font-medium text-cyan-950/70 dark:text-yellow-300"
						>Luz acesa</span
					>50h
				</h1>
