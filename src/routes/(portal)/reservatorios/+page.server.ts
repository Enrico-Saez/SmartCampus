...
async function getWaterTankDataDistance() {
	let resul = await fetch(
		'http://srv538573.hstgr.cloud:8888/api/timeseries/v0.1/smartcampusmaua/GetWaterTankLevel',
		{
			method: 'GET',
			headers: {
...
async function getHydrometerDataCounter() {
	let resul = await fetch(
		'http://srv538573.hstgr.cloud:8888/api/timeseries/v0.1/smartcampusmaua/GetHydrometer',
		{
			method: 'GET',
			headers: {
				'Content-Type': 'application/json'
...
async function getWaterTankInfo() {
	const [waterTankDataDistance, waterTankProperties, waterTankAccumulatedLiters] =
		await Promise.all([
			getWaterTankDataDistance(),
			getWaterTankProperties(),
			getHydrometerDataCounter()
		]);
	...
	if (
		Object.keys(waterTankDataDistance).length == 0 ||
		Object.keys(waterTankProperties).length == 0 ||
		Object.keys(waterTankAccumulatedLiters).length == 0
	) {
		return combinedData;
	}

	waterTankDataDistance.forEach(
		(item: { tags: { nodeName: string }; fields: { data_distance: number } }) => {
	...
	waterTankAccumulatedLiters.forEach(
		(item: { tags: { nodeName: string }; fields: { data_counter: number } }) => {
			let nodeName =
				'WaterTankLavel_' + item.tags.nodeName.substring(item.tags.nodeName.length - 1);
...
export const actions = {
	saveReservatoryData: async ({ request }) => {
		...
		if (data == undefined) {
			adminDBPortal
				.collection('reservatories')
				.doc(name!.toString())
				.create({ name: name, height: height, volume: volume });
		} else {
			adminDBPortal
				.collection('reservatories')
				.doc(name!.toString())
				.update({ name: name, height: height, volume: volume });
		}
		return { success: true };
	}
};
