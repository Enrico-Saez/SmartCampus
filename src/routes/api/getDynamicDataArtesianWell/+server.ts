...
async function getArtesianWellInfo() {
    ...
    let combinedData: { [key: string]: { name: string; data_pressure_0: number; data_pressure_1: number; error: number; } } = {};

    if (Object.keys(ArtesianWellDataDistance).length == 0) {
        return combinedData;
    }
    
    ArtesianWellDataDistance.forEach((item: { tags: { nodeName: any; }; fields: { data_pressure_0: any; data_pressure_1: any; }; }) => {
        ...
        if (!data_pressure_0 && !data_pressure_1) {
            combinedData[nodeName] = { name: nodeName, data_pressure_0: 0, data_pressure_1: 0, error:1};            
        } else {
