...
export const handle = (async ({ event, resolve }) => {
	...
	if (!event.locals.userID) {
		if ((!event.url.pathname.endsWith("/") && !event.url.pathname.startsWith("/api/login") && event.request.headers.get("X-Sveltekit-Action") == "true") && !event.url.pathname.startsWith("/api/telegramAlert")){
			throw redirect(303, '/');
		}
	}
